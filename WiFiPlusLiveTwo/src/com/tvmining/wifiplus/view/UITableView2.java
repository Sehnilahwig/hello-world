...
public class UITableView2 extends LinearLayout {
	...
	public UITableView2(Context context, AttributeSet attrs) {
		super(context, attrs);
	 ...
	 * @param selected
	 */
	public void addBasicItem(String title,boolean selected) {
		mItemList.add(new BasicItem(title,selected));
	}
	...
	private void setupBasicItem(View view, BasicItem item, int index) {
		...
		((ImageView) view.findViewById(R.id.chevron)).setImageResource(R.drawable.checked);
		if(item.isSelect()){
			((ImageView) view.findViewById(R.id.chevron)).setVisibility(View.VISIBLE);
		}else{
			((ImageView) view.findViewById(R.id.chevron)).setVisibility(View.INVISIBLE);
		}
