...
public class UITableViewUnClick extends LinearLayout {
	...
	public UITableViewUnClick(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public UITableViewUnClick(Context context) {
		super(context, null);
		mItemList = new ArrayList<IListItem>();
		mInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		mMainContainer = (LinearLayout)  mInflater.inflate(R.layout.list_container, null);
		...
		if(mItemList.size() > 1) {
			...
			for(IListItem obj : mItemList) {
				...
				if(mIndexController == 0) {
					tempItemView = mInflater.inflate(R.layout.list_item_top_unclick, null);
				}else if(mIndexController == mItemList.size()-1) {
					tempItemView = mInflater.inflate(R.layout.list_item_bottom_unclick, null);
				}else {
					tempItemView = mInflater.inflate(R.layout.list_item_middle_unclick, null);
				}	
	...
	private void setupBasicItem(View view, BasicItem item, int index) {
		...
		if(item.getSubtitle() != null) {
			((TextView) view.findViewById(R.id.itemCount)).setText(item.getSubtitle());
		}
		else {
			((TextView) view.findViewById(R.id.itemCount)).setVisibility(View.GONE);
		}		
		((TextView) view.findViewById(R.id.title)).setText(item.getTitle());
		TextPaint tp = ((TextView) view.findViewById(R.id.title)).getPaint(); 
